...
struct filename_info {
  ...
  int unchecked_letters;
  int puzzle_size;
  double unchecked_letters_pct;
};
...
static char usage[] = "usage: fgrid_unchecked_letters (-sort) (-ascending) filename\n";
...
int main(int argc,char **argv)
{
  ...
  for (n = 0; n < num_files; n++) {
    printf("%6.2lf%% unchecked letters (%d %d) %s\n",
      filename_inf[ixs[n]].unchecked_letters_pct,
      filename_inf[ixs[n]].unchecked_letters,
...
static int grid_info(char *filename)
{
  ...
  unchecked_letters = count_unchecked_letters(in_buf,width,height);

  strcpy(filename_inf[num_files].filename,filename);
  filename_inf[num_files].unchecked_letters = unchecked_letters;
  filename_inf[num_files].puzzle_size = puzzle_size;
  filename_inf[num_files].unchecked_letters_pct = (double)unchecked_letters / (double)puzzle_size * (double)100;
...
static int elem_compare(const void *elem1,const void *elem2)
{
  ...
  if (filename_inf[ix1].unchecked_letters_pct > filename_inf[ix2].unchecked_letters_pct)
    return 1;
  else if (filename_inf[ix1].unchecked_letters_pct < filename_inf[ix2].unchecked_letters_pct)
    return -1;
  else
    return 0;
}
