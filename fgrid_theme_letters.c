...
static char usage[] = "usage: fgrid_theme_letters (-sort) (-ascending) filename\n";
...
int main(int argc,char **argv)
{
  ...
  for (n = 0; n < num_files; n++) {
    printf("%6.2lf%% theme letters (%d %d) %s\n",
      filename_inf[ixs[n]].theme_letters_pct,
      filename_inf[ixs[n]].theme_letters,
...
static int grid_info(char *filename)
{
  ...
  filename_inf[num_files].theme_letters = theme_letters;
  filename_inf[num_files].puzzle_size = puzzle_size;
  filename_inf[num_files].theme_letters_pct = (double)theme_letters / (double)puzzle_size * (double)100;
...
static int elem_compare(const void *elem1,const void *elem2)
{
  ...
  if (filename_inf[ix1].theme_letters_pct > filename_inf[ix2].theme_letters_pct)
    return 1;
  else if (filename_inf[ix1].theme_letters_pct < filename_inf[ix2].theme_letters_pct)
    return -1;
  else
    return 0;
}
